...
| 2026-09-18 | Public submissions are **off** unless both Turnstile keys are set, and a failed check refuses rather than passes | It is the only unauthenticated door in the project: submitting hands an anonymous visitor a presigned URL to the bucket. Every other flag here degrades to worse-but-working; the degraded state of this one is "anyone may write to our storage". A missing secret or an unreachable verifier therefore refuses — the failure to design for is an environment variable dropped in a deploy, leaving the bucket open until somebody notices |
| 2026-09-18 | A public submission's `actorId` is **null**, not a service account | It genuinely has no account behind it, and writing one of ours in would be a false record of who acted. `audit_log.actor_id` is already nullable; who sent it, if they chose to say, goes in the entry's payload |
...
| 2026-09-19 | Topic descriptions are written by `category.describe`, never `category.rename`, and the bulk command only fills gaps | A description changes no name and no URL, so filing it as a rename would send anyone hunting a broken link to an entry that broke nothing — the same reasoning as `category.unmerge`. The bulk door refuses to overwrite because an existing description may be a person's own words typed into the panel, and a default replacing them is the wrong way round |
| 2026-09-19 | Every material cover follows the home page's books: one layout, a colour family and two scenes per topic, and each material's own edition and scene within them | The library had drifted into hundreds of unrelated looks, one hue per topic across the whole wheel. Colours are now fixed hex from a short list — cream, beige, sage, olive, forest, earth, charcoal, gold — and a test holds every one under a chroma limit, so nothing neon can be added by accident. Each family has a deep edition, so a shelf mixes light and dark books without leaving its family. Placed by hand for the twenty busiest topics, by name for the rest |
